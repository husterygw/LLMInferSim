...
阶段 9.5 (§10.5 8.5 FP8 收尾) 起新增:
  - dtype-aware: routed expert 用 `expert_w_byte` (通用 fp4 MoE: 0.5), 其他用 w_byte
...
def _count_attention_per_layer(model: ModelProfile) -> int:
    h = model.hidden_dim
...
def _count_dense_ffn_per_layer(model: ModelProfile) -> int:
    # SwiGLU: gate + up + down 三个 GEMM
    return 3 * model.hidden_dim * model.ffn_dim


def _count_routed_experts_per_layer(model: ModelProfile) -> int:
    """Routed experts 参数: n_routed × (gate + up + down) × per-expert dim.
    路由 gate 参数小 (h × n_routed), 一并算进去 (跟 MoE expert 主体相比 < 1%).
    """
    h = model.hidden_dim
...
def _count_shared_experts_per_layer(model: ModelProfile) -> int:
    h = model.hidden_dim
    return model.num_shared_experts * 3 * h * model.expert_dim


def estimate_param_count(model: ModelProfile) -> int:
