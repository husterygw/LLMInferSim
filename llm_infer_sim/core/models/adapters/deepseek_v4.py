"""DeepSeek-V4 family adapter (model_type=deepseek_v4) —— **config-only / migration reference**.

V4 runtime 不支持 (sparse attention / HC / hash MoE 已在 #157 删除, 需在新 operator class
architecture 上重做; profile_extractor 检测到 V4 sparse/HC config 会直接报错)。本 adapter 只做
hf_config 字段解析 (6 个基础 getter), 供 config ingest / sizing 复用, 不代表 runtime 可运行。
