...
from llm_infer_sim.core.operators.attention import (
    _flash_decode_spec, _flash_prefill_spec,
)
...
def _profiles_from_record(record: OperatorRecord):
    ...
    runtime_profile = RuntimeProfile.flat(
...
def _ctx_from_record(
    ...
    *,
    ...
    deployment, runtime_profile = _profiles_from_record(record)
...
def _attention_from_record(record: OperatorRecord, *, hw: HardwareConfig) -> Attention:
    # 从 DB record 的固定 shape 重建一个 baked Attention op (gap 分析按 record shape,
    # 不走 build-once 的 forward 重算)。
    shape = dict(record.signature.shape)
    runtime = dict(record.signature.runtime)
    ctx = _ctx_from_record(record, hw=hw)
    subtype = record.signature.op_subtype
    nq, nkv, hd, bs = (int(shape["num_q_heads"]), int(shape["num_kv_heads"]),
                       int(shape["head_dim"]), int(shape["num_seqs"]))
    byte = dict(a_byte=ctx.a_byte, kv_byte=ctx.kv_byte, onchip=ctx.hw.onchip_buffer)
    if subtype == "prefill":
        seqlen = int(shape["q_len"])
        spec = _flash_prefill_spec(seqlen=seqlen, bs=bs, n_q=nq, n_kv=nkv, head_dim=hd, **byte)
        q_len, kv_len, num_tokens = seqlen, seqlen, bs * seqlen
    elif subtype == "decode":
        kv_len = int(shape["kv_len"])
        spec = _flash_decode_spec(ctx_len=kv_len, bs=bs, n_q=nq, n_kv=nkv, head_dim=hd, **byte)
        q_len, num_tokens = 1, bs
    else:
        raise ValueError(f"unsupported attention subtype: {subtype!r}")
    return Attention(
        name=f"attention_{subtype}_{record.source.get('case_id')}",
        op_subtype=subtype, phase=subtype, layer_idx=0,
        num_tokens=num_tokens, num_seqs=bs, q_len=q_len, kv_len=kv_len,
        num_q_heads=nq, num_kv_heads=nkv, head_dim=hd,
        attention_backend="flash_attn", kv_dtype="bf16", block_size=ctx.block_size,
        kernel_source=runtime.get("kernel_source", record.kernel_source),
        ctx=ctx, roofline_spec_value=spec,
    )


def _moe_from_record(record: OperatorRecord, *, hw: HardwareConfig) -> MoE:
    shape = dict(record.signature.shape)
    runtime = dict(record.signature.runtime)
    model = ModelConfig(
        name="collector_moe",
...
def main() -> int:
    ...
    for record in records:
        ...
        if mode not in backends:
            backends[mode] = RooflineBackend(hw, mode)
