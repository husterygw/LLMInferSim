...
MODEL_ALIASES: dict[str, str] = {
    "qwen3_4b": "/data1/home/ygw268/models/Qwen3-4B-Instruct-2507",
    "qwen2_5_3b": "/data1/home/ygw268/models/Qwen2.5-3B-Instruct",
    "qwen3_32b": "/data1/home/ygw268/models/Qwen3-32B",
    "qwen3_30b_a3b": "/data1/home/ygw268/models/Qwen3-30B-A3B-Instruct-2507",
}
        ...
        for name, i, o in shapes:
            cases.append(
                _case(
                    ...
                    execution_mode="cudagraph",
                    max_model_len=32768,
                    # cap 32768 + gpu_mem 0.85: 防 RTX 4090 (24GB) real 启动失败.
                    # 原 max(32768, c*i) 让 c4×i16384=65536, KV cache 算到负数
                    # (No available memory for cache blocks). chunked_prefill 关,
                    # 单请求 16384 prefill 仍是 1 chunk ≤ 32768, 不影响 fit.
...
def build_moe_ep_batch_sweep() -> list[BenchCase]:
    suite = "moe_ep_batch_sweep"
    shapes = [
        ("i128_o128", 128, 128),
        ("i512_o128", 512, 128),
        ("i2048_o128", 2048, 128),
        ...
        for name, i, o in shapes:
            cases.append(
                _case(
                    ...
                    execution_mode="cudagraph",
                    max_model_len=8192,
                    # cap 32768 同 dense batch sweep, 防 4090 activation 爆显存.
                    max_num_batched_tokens=min(max(8192, c * i), 32768),
                    gpu_mem_util=0.85,
                    enable_expert_parallel=True,
                    tags=("moe", "ep", "batch", "tp4", "ep4", f"c{c}"),
                    description=f"Qwen3-30B-A3B TP=4 EP=4 MoE EP batch sweep c={c}, ISL={i}, OSL={o}",
                )
            )
    return cases
...
SUITE_BUILDERS = {
    ...
    "moe_ep_batch_sweep": build_moe_ep_batch_sweep,
