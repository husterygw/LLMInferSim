...
def test_flash_attn_maps_to_unified_ragged():
    ...
    profile = _extract_kernel_profile(cfg)
    assert isinstance(profile, KernelBackendProfile)
    assert profile.backend_name == "flash_attn"
...
def test_flashinfer_maps_to_unified_ragged():
    ...
    profile = _extract_kernel_profile(cfg)
    assert profile.backend_name == "flashinfer"
    assert profile.mixed_attention.mode == "unified_ragged"
...
def test_backend_none_uses_platform_default():
    ...
    profile = _extract_kernel_profile(cfg)
    assert profile.backend_name == "flash_attn_auto"
    assert profile.mixed_attention.mode == "unified_ragged"
...
def test_mla_backends_are_placeholders():
    ...
    for name in ("FLASH_ATTN_MLA", "FLASHMLA", "FLASHINFER_MLA", "TRITON_MLA"):
        profile = _extract_kernel_profile(_mock_vllm_config(name))
        assert profile.mixed_attention.mode == "unified_ragged", name
        assert profile.backend_name == name.lower()
...
def test_default_kernel_profile_still_works():
    """KernelBackendProfile() 默认供 standalone 模式, 不应受改动影响。"""
