...
def test_qwen3_4b_parse():
    ...
    scenario = extract_scenario(vllm_cfg)

    assert isinstance(scenario, SimulationScenario)
    m = scenario.model.to_legacy()
...
def test_quantization_defaults_fp16():
    """未量化模型: QuantizationProfile 默认 fp16 (w/a/kv = 2.0 byte)."""
    hf = SimpleNamespace(
        model_type="opt",
        num_attention_heads=12, hidden_size=768, num_hidden_layers=12,
        ffn_dim=3072, vocab_size=50272,
    )
    scenario = extract_scenario(_make_vllm_config(hf))
    assert scenario.model.quantization.w_byte == 2.0
    assert scenario.model.quantization.a_byte == 2.0
    assert scenario.model.quantization.kv_byte == 2.0


def test_unsupported_model_raises():
    import pytest

    from llm_infer_sim.core.models.adapters import UnsupportedModelError

    hf = SimpleNamespace(model_type="nonexistent_family")
    with pytest.raises(UnsupportedModelError):
        extract_scenario(_make_vllm_config(hf))
...
def test_qwen3_30b_a3b_moe_parse():
    ...
    scenario = extract_scenario(_make_vllm_config(hf, "Qwen/Qwen3-30B-A3B"))
    m = scenario.model.to_legacy()
...
_QWEN3_30B_A3B_PATH = Path("/data1/home/ygw268/models/Qwen3-30B-A3B-Instruct-2507/config.json")
_QWEN3_32B_PATH = Path("/data1/home/ygw268/models/Qwen3-32B/config.json")
...
def test_qwen3_30b_a3b_real_config_parse_is_moe():
    ...
    scenario = extract_scenario(_make_vllm_config(hf, "Qwen3-30B-A3B-Instruct-2507"))
    m = scenario.model.to_legacy()
...
def test_qwen3_32b_dense_alias_not_used_for_moe():
    ...
    scenario = extract_scenario(_make_vllm_config(hf, "Qwen3-32B"))
    m = scenario.model.to_legacy()
...
def test_deepseek_naming_still_works():
    ...
    scenario = extract_scenario(_make_vllm_config(hf, "fake-deepseek-like"))
    m = scenario.model.to_legacy()
    assert m.is_moe
...
def test_qwen_shared_expert_intermediate_size_derived():
    ...
    scenario = extract_scenario(_make_vllm_config(hf, "fake-qwen2-moe"))
    m = scenario.model.to_legacy()
    assert m.is_moe
...
def test_deepseek_v3_parse():
    ...
    scenario = extract_scenario(_make_vllm_config(hf, "deepseek-ai/DeepSeek-V3"))
    m = scenario.model.to_legacy()
    # 基础
...
def test_extracted_backend_profile_for_no_attention_config():
    ...
    scenario = extract_scenario(_make_vllm_config(hf))
    assert scenario.runtime.kernels.backend_name == "flash_attn_auto"
    assert scenario.runtime.kernels.mixed_attention.mode == "unified_ragged"
...
def test_quant_method_fp8_switches_w_byte_and_a_byte():
    ...
    scenario = extract_scenario(_make_vllm_config_with(quant_cfg=qcfg))
    assert scenario.model.quantization.w_byte == 1.0
    assert scenario.model.quantization.a_byte == 1.0
...
def test_quant_method_fp4_overrides_w_a_byte():
    ...
    scenario = extract_scenario(_make_vllm_config_with(quant_cfg=qcfg))
    assert scenario.model.quantization.w_byte == 0.5
    assert scenario.model.quantization.a_byte == 0.5


def test_no_quant_config_defaults_to_fp16():
    """无 quantization_config → 默认 fp16 (w_byte=a_byte=2.0)."""
    scenario = extract_scenario(_make_vllm_config_with())
    assert scenario.model.quantization.w_byte == 2.0
    assert scenario.model.quantization.a_byte == 2.0
...
def test_cache_dtype_fp8_switches_kv_byte():
    ...
    scenario = extract_scenario(_make_vllm_config_with(cache_dtype="fp8"))
    assert scenario.model.quantization.kv_byte == 1.0


def test_cache_dtype_fp8_e4m3_substring_match():
    """cache_dtype="fp8_e4m3" 子串匹配 → kv_byte=1.0."""
    scenario = extract_scenario(_make_vllm_config_with(cache_dtype="fp8_e4m3"))
    assert scenario.model.quantization.kv_byte == 1.0


def test_cache_dtype_auto_keeps_default():
    """cache_dtype="auto" → kv_byte 保持默认 fp16 (跟随 model dtype)."""
    scenario = extract_scenario(_make_vllm_config_with(cache_dtype="auto"))
    assert scenario.model.quantization.kv_byte == 2.0


def test_cache_dtype_int8_switches_kv_byte():
    scenario = extract_scenario(_make_vllm_config_with(cache_dtype="int8"))
    assert scenario.model.quantization.kv_byte == 1.0
