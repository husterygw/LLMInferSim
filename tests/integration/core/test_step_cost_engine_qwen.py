...
def test_decode_total_latency_smaller_than_prefill_for_same_isl():
    ...
    engine = build_qwen_dense_roofline_engine(
        model=_qwen3_4b(), deployment=DeploymentProfile.flat(),
        runtime=RuntimeProfile.flat(),
        hw=get_hardware_profile("RTX_4090"),
    )
...
def test_deploy_change_changes_parallel_metadata():
    ...
    engine_tp1 = build_qwen_dense_roofline_engine(
        model=model, deployment=DeploymentProfile.flat(tp=1),
        runtime=RuntimeProfile.flat(), hw=hw,
    )
    engine_tp2 = build_qwen_dense_roofline_engine(
        model=model, deployment=DeploymentProfile.flat(tp=2),
        runtime=RuntimeProfile.flat(), hw=hw,
    )
...
def test_execution_mode_change_affects_kernel_overhead():
    ...
    model = _qwen3_4b()
    hw = get_hardware_profile("RTX_4090")
    # Step G: kernel_overhead 经 CalibrationProfile 注入 (不再挂 hw spec).
    ...
    e_eager = build_qwen_dense_roofline_engine(
        model=model, deployment=DeploymentProfile.flat(),
        runtime=RuntimeProfile.flat(execution_mode="eager"), hw=hw, calibration=calib,
    )
    e_graph = build_qwen_dense_roofline_engine(
        model=model, deployment=DeploymentProfile.flat(),
        runtime=RuntimeProfile.flat(execution_mode="cudagraph"), hw=hw, calibration=calib,
    )
    wl = _prefill(isl=128)
...
def test_mixed_phase_smoke(engine):
    """mixed phase: 单个 attention op (op_subtype="mixed"), 不再拆 2 个 segment op.

    无 fallback, engine 不应抛; cost = prefill 段 + decode 段 合成 (一个 fused kernel).
    """
    wl = _mixed(isl=128, decode_n=4, decode_ctx=256)
    trace = engine.estimate(wl)
    assert trace.total_latency_s > 0
    assert trace.bottleneck in ("compute", "memory")
    # 单 attention op, count=num_layers, op_subtype="mixed"
    attn_entries = [
        e for e in trace.entries
        if e.op_kind == "attention" and e.metadata.get("count", 1) == 36
    ]
    assert len(attn_entries) == 1, (
        f"mixed 应只有 1 个 attention op (unified kernel), got {len(attn_entries)}"
    )
    assert attn_entries[0].op_subtype == "mixed"
