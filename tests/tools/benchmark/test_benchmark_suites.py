...
def test_single_tp1_roofline_suite_generation(tmp_path: Path):
    ...
    _run(sys.executable, "scripts/bench/bench_cases.py", "--suite", "single_tp1_roofline", "--out", str(out))
...
def test_batch_tp1_sweep_contains_expected_concurrency(tmp_path: Path):
    ...
    _run(sys.executable, "scripts/bench/bench_cases.py", "--suite", "batch_tp1_sweep", "--out", str(out))

    cases = _load_jsonl(out)
    # 9 shapes × concurrency {4,8,16,32}
    assert len(cases) == 36
    assert sorted({case["concurrency"] for case in cases}) == [4, 8, 16, 32]
...
def test_long_context_suite_bounds_vllm_scheduler_limits(tmp_path: Path):
    ...
    _run(sys.executable, "scripts/bench/bench_cases.py", "--suite", "long_context_sweep", "--out", str(out))

    cases = _load_jsonl(out)
    assert cases
    assert all(case["max_model_len"] == 32768 for case in cases)
    # max_num_batched_tokens capped at 32768 + gpu_mem 0.85 to avoid RTX 4090 (24GB)
    # real-server OOM (No available memory for cache blocks). The old
    # `>= concurrency * input_len` invariant no longer holds (c4×i16384=65536 > cap);
    # with chunked_prefill off, a single request's prefill must still fit the cap.
    assert all(case["max_num_batched_tokens"] == 32768 for case in cases)
    assert all(case["gpu_mem_util"] == 0.85 for case in cases)
    assert all(case["input_len"] <= case["max_num_batched_tokens"] for case in cases)
...
def test_bench_compare_dry_run_prints_server_and_bench_commands(tmp_path: Path):
    ...
    result = _run(
        "bash",
        "scripts/bench/bench_compare.sh",
        "--cases",
        str(cases),
        "--out",
        str(out),
...
def test_run_bench_suite_legacy_alias_dry_run(tmp_path: Path):
    result = _run(
        "bash",
        "scripts/bench/run_bench_suite.sh",
        "A",
        "--filter-case",
        "*i128_o128*",
        "--dry-run",
