...
def _estimate(bundle, workload) -> StepCostTrace:
    engine = build_qwen_roofline_engine(bundle.model, bundle.deploy, bundle.hw)
...
def test_v3_estimate_produces_step_cost_trace(wl):
    bundle = extract_profile_bundle(_qwen3_4b_vc())
    trace = _estimate(bundle, wl)
...
def test_v3_handles_mixed_phase():
    bundle = extract_profile_bundle(_qwen3_4b_vc())
    trace = _estimate(bundle, _mixed_wl())
    assert trace.total_latency_s > 0
...
def test_v3_large_decode_batch():
    bundle = extract_profile_bundle(_qwen3_4b_vc())
    trace = _estimate(bundle, _decode_wl(n=64, ctx=4096))
    assert trace.total_latency_s > 0
...
def test_v3_trace_entries_populated():
    bundle = extract_profile_bundle(_qwen3_4b_vc())
    trace = _estimate(bundle, _prefill_wl(128))
...
def test_v3_decode_bottleneck_is_memory():
    bundle = extract_profile_bundle(_qwen3_4b_vc())
    trace = _estimate(bundle, _decode_wl(n=1, ctx=512))
    assert trace.bottleneck == "memory"


def test_v3_long_prefill_bottleneck_is_compute():
    bundle = extract_profile_bundle(_qwen3_4b_vc())
    trace = _estimate(bundle, _prefill_wl(2048))
