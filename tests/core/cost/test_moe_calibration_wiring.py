...
from llm_infer_sim.core.operators import (
    AllReduce, ElementWise, GEMM, MoE, MoEDispatch, MoERoutingProfile,
)
...
from llm_infer_sim.core.profiles.moe_efficiency import MoEEfficiencyProfile
...
def _qwen_moe_ctx(tp: int = 4, ep: int = 1):
    ...
    return mc, deploy, build_operator_context(mc, deploy, get_hardware_profile("RTX_4090"))


def _hw_with_profile(profile: MoEEfficiencyProfile):
    hw = get_hardware_profile("RTX_4090")
    return dataclasses.replace(hw, moe_efficiency=profile)


def _estimate(op, hw, deploy):
...
def test_local_dispatch_overhead_changes_only_dispatch_latency():
    ...
    mc, deploy, ctx = _qwen_moe_ctx()
    md_op = MoEDispatch.local(
        pre_dispatch=True, layer_idx=0, tokens=128, ctx=ctx, phase="decode",
    )
...
def test_moe_efficiency_none_skips_calibration():
    """hw.moe_efficiency=None 时 latency 跟 roofline 一致, 不写 MoE metadata 字段."""
    mc, deploy, ctx = _qwen_moe_ctx()
    moe_op = MoE.routed_experts(
        layer_idx=0, tokens=128, ctx=ctx,
        routing=MoERoutingProfile.balanced(), phase="prefill",
    )
    hw_none = dataclasses.replace(get_hardware_profile("RTX_4090"), moe_efficiency=None)
    e = _estimate(moe_op, hw_none, deploy)
