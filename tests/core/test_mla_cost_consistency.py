...
def _deepseek_v3(tp: int = 8) -> tuple[ModelConfig, DeployConfig]:
    model = ModelConfig(
        name="DeepSeek-V3",
    ...
    deploy = DeployConfig(tp_size=tp)
    return model, deploy


def _v3_lite_no_q_lora(tp: int = 1) -> tuple[ModelConfig, DeployConfig]:
...
def _layer_ops(model: ModelConfig, deploy: DeployConfig, *,
...
def test_mla_without_q_lora_uses_single_q_proj():
    ...
    m, deploy = _v3_lite_no_q_lora(tp=1)
...
def test_mla_prefill_chunked_reads_c_kv_cache():
    ...
    m, deploy = _deepseek_v3(tp=8)
    ops = _chunked_prefill_layer_ops(m, deploy, current_tokens=128, total_ctx=1024)
    attn = next(op for op in ops if op.op_kind == "attention")
...
def test_non_mla_prefill_kv_io_unchanged():
    ...
    hw = get_hardware_profile("RTX_4090")
    engine = build_qwen_dense_roofline_engine(qwen, deploy, hw)
    wl = GlobalStepWorkload(
        step_id=0, phase=StepPhase.PREFILL,
        requests=[RequestWorkload(
            request_id="r0", phase=StepPhase.PREFILL,
